#!/usr/bin/env bun
/**
 * 调试脚本：打印完整的 Craft Agent system prompt，并附带说明注释。
 *
 * 作用：把静态 system prompt 和每条用户消息前动态注入的上下文组件展示出来，
 * 方便理解 prompt 的拼接方式。
 *
 * 运行方式：bun run print:system-prompt
 */
...
// ANSI 颜色码，用于在终端输出高亮。格式和 Go 的转义字符串类似。
const colors = {
  reset: '\x1b[0m',
  bold: '\x1b[1m',
  dim: '\x1b[2m',
...
const systemPrompt = getSystemPrompt(
  undefined, // 没有固定偏好，使用磁盘上的当前偏好
  { enabled: false }, // 关闭调试模式，输出更干净
  '/Users/example/.craft-agent/workspaces/abc123' // 示例工作区路径
);
...
// 3. Source 状态（示例；因为需要 agent 实例，这里直接写示例字符串）
...
// 4. 工作区能力（workspace capabilities）
...
// 6. 恢复上下文（recovery context，仅在会话恢复时出现）
